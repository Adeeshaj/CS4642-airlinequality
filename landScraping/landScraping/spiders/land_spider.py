...
class landsSpider(scrapy.Spider):
	...
	def start_requests(self):
		url = "https://www.lankapropertyweb.com/land/index.php?page="
		urls = []
		for i in range(50):
			y = url + str(i+1)+"/"
			urls.append(y)
	...
	def parse(self, response):
		...
		for url in response.css("article.item div.thumbnail div.caption1 h4.listing-titles"):
			...
			yield scrapy.Request(url=inner_url, callback=self.parse_land)
	...
	def parse_land(self, response):
		...
		with open(file_name, 'w') as file:
		    file.write(json.dumps(land_entry, indent=2))
